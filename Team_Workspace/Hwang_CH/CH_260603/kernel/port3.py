...
# 2. _enrich_place_info 메서드 전체 교체 (DB 조회 -> qust.sDB 메모리 조회로 변경)
    def _enrich_place_info(self, t_pn_raw: list | None, sdb_items: list | None) -> list | None:
        """LLM이 생성한 place_info를 qust.sDB 실제 데이터로 교정 (DB 접근 X)."""
        if not t_pn_raw or not sdb_items:
            return t_pn_raw
            
        # 일차별 다차원 딕셔너리를 평탄화하여 이름 기반 단일 딕셔너리로 변환
        sdb_dict = {}
        if isinstance(sdb_items, dict):
            ...
            sdb_dict = {item.name: item for item in sdb_items}
        ...
        for day in t_pn_raw:
            ...
            for item in day:
                ...
                # qust.sDB 목록에 해당 장소가 있으면 정보 교정
                if place in sdb_dict:
                    row = sdb_dict[place]
                    item["place_info"] = {
                        "name":         row.name,
