...
class WidgetUnit:
    ...
    async def get_t_sl_front(session_id: str, redis: Any) -> dict:
        ...
        widget_state = await Cacher.get_session_widgets(session_id, redis)
        value = widget_state.get("t_sl", "")
        if not value:
            value = await TripSelectWidget.load_from_redis(session_id, redis)
        return TripSelectWidget.format_for_front(value)
