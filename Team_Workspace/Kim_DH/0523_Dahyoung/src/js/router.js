...
async function _renderTripSelect(chatHistory, ssid) {
  const data = await BackendHooks.fetchTripSelect(ssid);
  ...
  if (path.startsWith('#/chat/')) {
    ...
    if (state.currentSessionId !== ssid) {
      ...
      try {
        ...
        await _renderTripSelect(chatHistory, ssid).catch(() => {});
          ...
          if (event.type === 'message') {
            if (event.sender_id !== myId) {
              ...
              if (event.sender_id === 'bot') {
                _renderTripSelect(chatHistory, ssid).catch(() => {});
              }
              chatHistory.scrollTop = chatHistory.scrollHeight;
