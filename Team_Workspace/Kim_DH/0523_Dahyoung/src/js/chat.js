...
export const ChatManager = {
    ...
    try {
      await BackendHooks.sendTeamMessage(
        ...
        () => {
          ...
          this._renderTripSelect(state, elements).catch(() => {});
        }
      );
    } catch (error) {
      removeLoading();
  ...
  async _renderTripSelect(state, elements) {
    if (!state.currentSessionId || state.isTempMode) return;
    const data = await BackendHooks.fetchTripSelect(state.currentSessionId);
    appendTripSelect(elements.chatHistory, data);
  },
