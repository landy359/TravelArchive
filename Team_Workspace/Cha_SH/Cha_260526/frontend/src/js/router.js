      ...
      state._sseConnection = BackendHooks.subscribeToSessionEvents(
        ...
        (event) => {
          if (event.type === 'message') {
            ...
            // 계획(T_PN) 업데이트 처리
            if (event.widgets && event.widgets.t_pn) {
              const planContent = document.querySelector('[data-plan-content]');
              if (planContent) {
                planContent.innerHTML = '';
                const items = event.widgets.t_pn;
                if (items.length === 0) {
                  planContent.innerHTML = '<div class="rs-plan-empty">등록된 계획이 없습니다.</div>';
                } else {
                  // 날짜별 그룹핑 시도 (YYMMDD 형식)
                  const groups = {};
                  items.forEach(item => {
                    const d = item.date || '000000';
                  ...
                  Object.keys(groups).sort().forEach(date => {
                    ...
                    dayDiv.className = 'rs-plan-day-group';
                    
                    const dateTitle = document.createElement('div');
                    dateTitle.className = 'rs-plan-date-title';
                    const y = date.substring(0,2), m = date.substring(2,4), d = date.substring(4,6);
                    dateTitle.textContent = date === '000000' ? '미정' : `20${y}년 ${m}월 ${d}일`;
                    dayDiv.appendChild(dateTitle);

                    groups[date].sort((a,b) => (a.order || 0) - (b.order || 0)).forEach(item => {
                      const itemDiv = document.createElement('div');
                      itemDiv.className = 'rs-plan-item';
                      itemDiv.innerHTML = `
                        <span class="rs-plan-order">${(item.order || 0) + 1}</span>
                        <span class="rs-plan-place">${item.place}</span>
                      `;
                ...
                // 계획 박스가 닫혀있다면 열어주기 (시각적 피드백)
                const planBox = document.getElementById('rs-plan-box');
                if (planBox && !planBox.classList.contains('rs-open')) {
                  planBox.classList.add('rs-open');
                  const toggle = planBox.querySelector('[data-plan-toggle]');
                  if (toggle) toggle.classList.add('rs-chevron-rotated');
                }
              }
            }
