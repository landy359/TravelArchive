    ...
    async def _stream_bot_response(session_id: str, query: str, triggering_user_id: str, redis: Any, manager: Any) -> StreamingResponse:
        ...
        async def _stream():
            ...
            try:
                ...
                # Broadcast widget updates via SSE
                if new_widgets:
                    from ...router.protocol import _mk_to_list
                    front_widgets = {}
                    if "t_mk" in new_widgets:
                        from ..widget.widget_trip_marker import TripMarkerWidget
                        tmw = TripMarkerWidget()
                        tmw.set_for_llm(new_widgets["t_mk"])
                        front_widgets["t_mk"] = tmw.get_for_front()
                    if "t_mp" in new_widgets:
                        front_widgets["t_mp"] = new_widgets["t_mp"]
                    if "t_pn" in new_widgets:
                        from ..widget.widget_trip_plan import TripPlanWidget
                        tpw = TripPlanWidget()
                        tpw.set_for_llm(new_widgets["t_pn"])
                        front_widgets["t_pn"] = tpw.get_for_front()
