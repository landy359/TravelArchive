...
async def add_map_marker(session_id: str, req: MapMarkerAddRequest, request: Request, user_id: str = Depends(get_current_user)):
    ...
    markers_raw = await WidgetUnit.get_markers(session_id, request.app.state.redis)
    widget = TripMarkerWidget()
    widget.set_for_llm(markers_raw) # Redis stores LLM format
    front_markers = widget.get_for_front()
    front_markers.append({"marker_id": req.marker_id, "lat": req.lat, "lng": req.lng, "title": req.title or ""})
    widget.set_for_front(front_markers)
    await WidgetUnit.set_markers(session_id, request.app.state.redis, widget.get_for_llm())
...
async def delete_map_marker(session_id: str, marker_id: str, request: Request, user_id: str = Depends(get_current_user)):
    ...
    markers_raw = await WidgetUnit.get_markers(session_id, request.app.state.redis)
    widget = TripMarkerWidget()
    widget.set_for_llm(markers_raw)
    front_markers = [m for m in widget.get_for_front() if m.get("marker_id") != marker_id]
    widget.set_for_front(front_markers)
    await WidgetUnit.set_markers(session_id, request.app.state.redis, widget.get_for_llm())
    return {"success": True}
...
async def save_map_markers(session_id: str, req: MapMarkersRequest, request: Request, user_id: str = Depends(get_current_user)):
    from .execute_unit.widget.widget_trip_marker import TripMarkerWidget
    widget = TripMarkerWidget()
    widget.set_for_front(req.markers)
    await WidgetUnit.set_markers(session_id, request.app.state.redis, widget.get_for_llm())
    return {"success": True}
...
async def get_map_markers(session_id: str, request: Request, user_id: str = Depends(get_current_user)):
    ...
    widget = TripMarkerWidget()
    widget.set_for_llm(markers_raw)
    return {"markers": widget.get_for_front()}
