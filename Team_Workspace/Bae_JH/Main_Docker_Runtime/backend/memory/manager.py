...
class EventHandler:
    ...
    async def _on_get_user_by_nickname(self, e: GetUserByNicknameEvent) -> None:
        try:
            ...
            if user_id:
                await self._redis.execute({"action": "set", "key": f"nick:{e.nickname}:user_id", "value": user_id, "ttl": USER_DATA_TTL})
    ...
    async def _on_create_session(self, e: CreateSessionEvent) -> None:
        try:
            ...
            await self._redis.execute({"action": "delete", "key": f"user:{e.user_id}:sessions:{e.data.get('trip_id', 'all')}"})
            await self._redis.execute({"action": "delete", "key": f"user:{e.user_id}:sessions:all"})
            e.future.set_result(None)
        except Exception as ex:
            e.future.set_exception(ex)
    ...
    async def _on_leave_as_master(self, e: LeaveAsMasterEvent) -> None:
        try:
            e.future.set_result(await Loader.leave_as_master(self._pg, e.session_id, e.user_id))
        except Exception as ex:
            e.future.set_exception(ex)
    ...
    async def _on_remove_non_master(self, e: RemoveNonMasterEvent) -> None:
        try:
            ...
            await self._redis.execute({"action": "delete", "key": f"session:{e.session_id}:participants"})
        except Exception as ex:
    ...
    async def _on_move_session_trip(self, e: MoveSessionTripEvent) -> None:
        try:
            e.future.set_result(await Loader.move_session_to_trip(self._pg, e.session_id, e.trip_id, e.user_id))
        except Exception as ex:
            e.future.set_exception(ex)
    ...
    async def _on_create_trip(self, e: CreateTripEvent) -> None:
        try:
            ...
            await self._redis.execute({"action": "delete", "key": f"user:{e.user_id}:trips"})
            e.future.set_result(result)
        except Exception as ex:
            e.future.set_exception(ex)
    ...
    async def _on_create_team(self, e: CreateTeamEvent) -> None:
        try:
            ...
            await self._redis.execute({"action": "delete", "key": f"user:{e.user_id}:teams"})
            e.future.set_result(result)
        except Exception as ex:
            e.future.set_exception(ex)
    ...
    async def _on_accept_invite(self, e: AcceptInviteEvent) -> None:
        try:
            ...
            await self._redis.execute({"action": "delete", "key": f"user:{e.user_id}:notifications"})
            await self._redis.execute({"action": "delete", "key": f"user:{e.user_id}:sessions:all"})
            e.future.set_result(result)
        except Exception as ex:
            e.future.set_exception(ex)
    ...
    async def _idle_sweep(self) -> None:
        result = await self._redis.execute({"action": "scan", "pattern": "session:*:dirty_widgets"})
