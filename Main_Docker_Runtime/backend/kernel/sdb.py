...
# 메인 검색: 키워드 있으면 name 매칭되는 row 만.
# places 테이블 실제 컬럼: address(→address_road), place_type(→main_category), geom
_MAIN_QUERY = f"""
WITH ranked AS (
    SELECT
        p.name,
        p.address         AS address_road,
        p.place_type      AS main_category,
        ROW_NUMBER() OVER (
            PARTITION BY p.place_type
    ...
    FROM {SDB_TABLE} p
    WHERE p.place_type = ANY(%(categories)s)
...
class SDB:
    ...
    async def _resolve_anchor(self, region: str) -> tuple[float, float]:
        ...
        return DEFAULT_CANDIDATES[0]  # 기본값: 제주시 (북부)
    ...
    async def run(self, qust: "QUST") -> "QUST":
        ...
        # T_CD가 있으면 그 날짜 수를 우선 사용 (메시지 추출값보다 정확)
        if qust.T_CD:
